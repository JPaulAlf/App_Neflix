...
public class Validacion implements Serializable{
    ...
    // PENSAR COMO HACER PERSISITENTES LAS DESCARGAS ENTRE LAS VENTANAS****LISTO
    ...
    public static String numerosLetras(int num){
        if(num >2000000)
            return "DOS MILLONES";
        ...
        if(num<20){
            return "DIECI"+ numerosLetras(num-10);
        }
        if(num<30){
            return "VEINTI" + numerosLetras(num-20);
        }
        if(num<100){
            return numerosLetras((int)(num/10)*10 ) + " Y " + numerosLetras(num%10);
        }        
        if(num<200){
            return "CIENTO " + numerosLetras(num - 100 );
        }         
        if(num<1000){
            return numerosLetras((int)(num/100)*100 ) + " " + numerosLetras(num%100);
        } 
        if(num<2000){
            return "MIL " + numerosLetras(num % 1000 );
        } 
        if(num<1000000){
            String h="";
            h = numerosLetras((int)(num/1000)) + " MIL" ;
            if(num% 1000!=0){
                h += " " + numerosLetras(num % 1000);
            }
            return h;
        }
        if(num<2000000){
            return "UN MILLON " + numerosLetras(num % 1000000 );
        } 
        return "";  
    }
...
//                ContenidoAudioVisual.agregarContenidoAudiovisual(contenidop);
    ...
    // Unicamemente los rellena una vez, si este al abrir detecta que falta alguno
    // de los siguientes, este lo vuelve a rellenar
